        ...
        <div>
          {groups.map((group) => (
            <section key={group.id} className="border-b last:border-b-0" style={{ borderColor: "var(--border)" }}>
              ...
              {group.users.map((u) => <UserEditorRow key={u.id} user={u} currentUserId={currentUserId} instituciones={instituciones} areas={areas} busy={busy} onPatch={patch} onPatchAreas={patchAreas} />)}
            </section>
          ))}
          {filtered.length === 0 && (
...
function UserEditorRow({ user, currentUserId, instituciones, areas, busy, onPatch, onPatchAreas }: {
  user: UserRow; currentUserId: string; instituciones: Inst[]; areas: AreaOption[]; busy: string | null;
  onPatch: (id: string, patch: Partial<Pick<UserRow, "rol" | "institucion_id">>) => void;
  onPatchAreas: (id: string, areaIds: string[]) => void;
}) {
  const isSelf = user.id === currentUserId;
  return <div className="grid grid-cols-1 gap-3 border-t px-5 py-3.5 first:border-t-0 md:grid-cols-[minmax(0,1.45fr)_minmax(132px,0.8fr)_minmax(170px,1fr)_minmax(160px,1fr)_28px] md:items-center md:gap-3" style={{ borderColor: "var(--border)" }}>
    ...
    <div className="flex min-w-0 items-center gap-1.5"><select className={`${selCls} min-w-0`} style={selStyle} value={user.rol} disabled={isSelf || busy === user.id} onChange={(event) => onPatch(user.id, { rol: event.target.value as UserRole })} title={isSelf ? "No puedes cambiar tu propio rol" : undefined}>{ROLES.map((role) => <option key={role} value={role}>{ROLE_LABEL[role]}</option>)}</select>{user.rol === "admin" && <ShieldCheck className="h-3.5 w-3.5 shrink-0" style={{ color: "var(--accent)" }} />}</div>
    <select className={`${selCls} w-full`} style={selStyle} value={user.institucion_id ?? ""} disabled={busy === user.id} onChange={(event) => onPatch(user.id, { institucion_id: event.target.value || null })}><option value="">— Sin institución —</option>{instituciones.map((institution) => <option key={institution.id} value={institution.id}>{institution.nombre}</option>)}</select>
    <AreaPicker user={user} areas={areas.filter((area) => area.institucion_id === user.institucion_id)} disabled={busy === user.id} onChange={(areaIds) => onPatchAreas(user.id, areaIds)} />
