...
export default async function QuizBuilderProfesorPage({ params }: { params: Promise<{ id: string; leccionId: string }> }) {
  const { id, leccionId } = await params;
  const user = await requireRole(["profesor", "admin"], `/profesor/cursos/${id}/leccion/${leccionId}/quiz`);
      ...
      <div className="mx-auto max-w-3xl px-5 py-10 sm:px-8 sm:py-12">
        ...
        <div className="animate-rise mb-6">
          ...
          <h1 className="mt-1 font-serif-brand tracking-tight"
            style={{ fontSize: "clamp(1.6rem,4vw,2.1rem)", color: "var(--text)" }}>{leccion.titulo}</h1>
        </div>
        <div className="animate-rise rise-2">
          <QuizBuilder cursoId={id} leccionId={leccionId} leccionTitulo={leccion.titulo} quiz={quiz} preguntas={preguntas} />
        </div>
      </div>
    </AppShell>
  );
