...
import { BookOpen, Plus, ArrowRight, Pencil, Users } from "lucide-react";
...
export default async function ProfesorCursosPage() {
  ...
  let cursos: Pick<Curso, "id" | "titulo" | "categoria" | "nivel" | "estado">[] = [];
  ...
  if (supabase) {
    ...
    if (cursos.length) {
      const { data: ins } = await supabase
        .from("inscripciones").select("curso_id").in("curso_id", cursos.map((c) => c.id));
      ((ins as { curso_id: string }[] | null) ?? []).forEach((i) =>
        inscritosPorCurso.set(i.curso_id, (inscritosPorCurso.get(i.curso_id) ?? 0) + 1));
    }
  }
    ...
    <AppShell user={user}>
      <div className="mx-auto max-w-5xl px-5 py-10 sm:px-8 sm:py-12">
        <div className="animate-rise mb-8 flex items-end justify-between gap-4">
          ...
          <Link href="/profesor/cursos/nuevo"
              ...
              return (
                <Link key={c.id} href={`/profesor/cursos/${c.id}`}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="flex items-center gap-2 truncate text-xs" style={{ color: "var(--text-faint)" }}>
                      {[c.categoria, c.nivel].filter(Boolean).join(" · ") || "Sin detalles"}
                      <span className="inline-flex items-center gap-1"><Users className="h-3 w-3" /> {inscritosPorCurso.get(c.id) ?? 0}</span>
                    </p>
                  </div>
                  ...
                  <Pencil className="h-4 w-4 shrink-0 opacity-0 transition-opacity group-hover:opacity-100" style={{ color: "var(--text-faint)" }} />
