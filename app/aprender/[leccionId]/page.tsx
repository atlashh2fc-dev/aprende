...
export default async function LeccionPage({
  ...
  const [{ data: cursoRaw }, { data: hermanasRaw }] = await Promise.all([
    supabase.from("cursos").select("slug, titulo, profesor_id").eq("id", leccion.curso_id).single(),
    supabase.from("lecciones").select("id, titulo, orden").eq("curso_id", leccion.curso_id).order("orden"),
  ]);
  const curso = cursoRaw as Pick<Curso, "slug" | "titulo" | "profesor_id"> | null;
  ...
  const initialNote = vistaAlumno ? "" : (noteRaw as { contenido: string } | null)?.contenido ?? "";
  ...
  if (leccion.tipo === "quiz") {
    ...
    if (q) {
      ...
      const { data: intentosRaw, count } = vistaAlumno
        ? { data: null, count: 0 }
        : await supabase
          .from("quiz_intentos")
    ...
    <AppShell user={user}>
      <div className="mx-auto max-w-6xl px-5 py-10 sm:px-8 sm:py-12">
        <div className="animate-rise">
          {curso && (
            <Link href={`/cursos/${curso.slug}${vistaAlumno ? "?vista=alumno" : ""}`}
          ...
          {vistaAlumno && (
            <p className="mt-4 rounded-xl px-4 py-3 text-xs leading-relaxed" style={{ background: "var(--primary-dim)", color: "var(--text-muted)", border: "1px solid var(--border)" }}>
              Vista alumno: el contenido se muestra tal como lo verá un estudiante. No se guardarán avances, notas ni respuestas.
            </p>
          )}
        ...
        {(anterior || siguiente) && (
          <div className="animate-rise rise-3 mt-10 flex items-stretch justify-between gap-3 border-t pt-6"
            ...
            {anterior ? (
              <Link href={`/aprender/${anterior.id}${vistaAlumno ? "?vista=alumno" : ""}`}
            ...
            {siguiente && (
              <Link href={`/aprender/${siguiente.id}${vistaAlumno ? "?vista=alumno" : ""}`}
